    ...
    if flags["storyboard"]:
        ...
        if shots:
            for shot in shots:
                ...
                if chars_field:
                    shot["characters"] = "+".join(char_id_map.get(c.strip(), c.strip()) for c in chars_field.split("+") if c.strip())
                scene_field = shot.get("scene_id", "")
                if scene_field in scene_id_map:
                    shot["scene_id"] = scene_id_map[scene_field]
