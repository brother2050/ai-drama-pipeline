...
class TestLLMGenerator:
    ...
    def test_generate_storyboard(self):
        ...
        class FakeLLM:
            def chat(self, prompt, system="", **kw):
                return json.dumps([
                    {"shot_id": "001", "scene_id": "room", "characters": "alice",
        ...
        shots = generate_storyboard(FakeLLM(), StoryboardGenParams(
    ...
    def test_generate_characters(self):
        ...
        class FakeLLM:
            def chat(self, prompt, system="", **kw):
                return json.dumps([{"id": "alice", "name": "Alice", "gender": "female",
                                    "appearance": "young woman", "outfits": {"default": {"description": "dress"}},
                                    "bible": {"core_traits": "kind"}}])

        chars = generate_characters(FakeLLM(), ["22岁温柔女生"], expected_ids=["alice"])
        assert len(chars) == 1
    ...
    def test_generate_scenes(self):
        ...
        class FakeLLM:
            def chat(self, prompt, system="", **kw):
                return json.dumps([{"id": "room", "name": "客厅", "description": "modern room", "lighting": "bright"}])

        scenes = generate_scenes(FakeLLM(), ["现代客厅"])
        assert len(scenes) == 1
...
class TestShotUtils:
    ...
    def test_validate_shot(self):
        ...
        assert validate_shot({"episode": 1, "shot_id": "001", "scene_id": "s",
                              "characters": "c", "action": "a", "dialogue": "d"}) == []
        errors = validate_shot({"episode": 1, "shot_id": "001", "scene_id": "s",
...
class TestConsistencyChecker:
    ...
    def test_scene_exists(self):
        ...
        from engines.consistency_checker import check_consistency
        errors = check_consistency(
            [{"shot_id": "001", "scene_id": "nonexistent", "duration": 4}],
            scenes=[{"id": "room"}])
        assert any("nonexistent" in e for e in errors)
    ...
    def test_outfit_continuity(self):
        ...
        from engines.consistency_checker import check_consistency
        errors = check_consistency([
            {"shot_id": "001", "scene_id": "s", "characters": "c", "outfit": "a", "duration": 4},
            {"shot_id": "002", "scene_id": "s", "characters": "c", "outfit": "b", "duration": 4},
        ])
...
class TestMultiCharacter:
    ...
    def test_calculate_regions(self):
        ...
        from engines.multi_char import MultiCharacterHandler
        h = MultiCharacterHandler()
        assert h.calculate_regions(0) == [{"position": "center", "x": 0.5, "y": 0.5}]
        assert len(h.calculate_regions(3)) == 3
...
class TestImportSystem:
    ...
    def test_import_plan_validation(self):
        ...
        plan = ImportPlan(
            ...
            shots=[{"shot_id": "001", "scene_id": "s1", "characters": "c1", "action": "Alice walks in slowly"}],
        )
    ...
    def test_import_plan_missing_ref(self):
        ...
        plan = ImportPlan(
            characters=[],
            scenes=[],
            shots=[{"shot_id": "001", "scene_id": "s1", "characters": "nonexistent", "action": "walks in slowly"}],
        )
        errors = ImportValidator.validate_references(plan)
        assert any("nonexistent" in e for e in errors)
    ...
    def test_import_plan_duplicate_shot_id(self):
        ...
        plan = ImportPlan(
            shots=[
                {"shot_id": "001", "scene_id": "s1", "action": "walks in slowly"},
                {"shot_id": "001", "scene_id": "s1", "action": "sits down"},
            ],
        )
        errors = ImportValidator.validate_references(plan)
        assert any("重复" in e for e in errors)
...
class TestModelRegistry:
    ...
    def test_registry_backends(self):
        ...
        assert "mimo-voicedesign" in tts
...
class TestServiceRegistry:
    ...
    def test_registry_create(self):
        ...
        from api import _ensure_registered
        _ensure_registered()
        config = {"models": {"tts_backend": "mimo-voicedesign"}, "timeouts": {}, "project_dir": ""}
        backend = registry.create("tts", "mimo-voicedesign", config)
