...
def test_reference_errors():
    ...
    plan_data = {
        ...
        "shots": [
            {"shot_id": "001", "scene_id": "office", "characters": "guchen", "action": "顾辰坐在办公桌前看文件，眉头紧锁"},
            {"shot_id": "001", "scene_id": "living_room", "characters": "linxia", "action": "林夏坐在沙发上翻手机"}
        ]
    }
    ...
    # 缺失的角色/场景也应报错
    assert any("office" in e and "不存在" in e for e in errors), f"预期场景缺失错误，实际: {errors}"
    assert any("guchen" in e and "不存在" in e for e in errors), f"预期角色缺失错误，实际: {errors}"
...
def test_shots_only_with_existing_project():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        plan = ImportPlan(**{
            "shots": [
                {"shot_id": "001", "scene_id": "rainy_night_alley", "characters": "lu_zhou", "action": "陆舟独自走在雨巷中，雨水打湿了他的肩膀"}
            ]
        })
