  ...
  try {
    ...
    if (_sbViewMode === 'timeline') {
        ...
        return `<div class="timeline-item" id="tl-${esc(sid)}"><div class="timeline-dot"></div>
          <div class="timeline-card" style="position:relative">
            ...
            <div class="timeline-info">
              <div class="timeline-info-head"><span class="timeline-sid">${esc(sid)}</span><span class="timeline-meta">${esc(_resolveScene(s.scene_id))} · ${esc(_resolveChars(s.characters))}</span></div>
      ...
      const rows = ss.map((s, i) => `<tr>
        ...
        <td>${_shotId(s, i)}</td>
        ${SB_FIELDS.slice(0, 4).map(f => { const _tip = f === 'characters' ? _resolveChars(s[f]) : f === 'scene_id' ? _resolveScene(s[f]) : ''; return `<td><input class="sb-inline-input" value="${esc(s[f] || '')}" data-idx="${i}" data-field="${f}"${_tip ? ` title="${esc(_tip)}"` : ''} onchange="updateShotField(this)"></td>`; }).join('')}
