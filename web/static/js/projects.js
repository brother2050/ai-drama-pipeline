...
async function loadProjects() {
  ...
  try {
    ...
    el.innerHTML = `<div class="card"><div style="display:flex;justify-content:space-between;margin-bottom:1rem"><h2>${t('proj.title')}</h2><div><button class="btn btn-success" onclick="newProj()">+ ${t('btn.add')}</button> <button class="btn btn-primary" onclick="importJsonProj()" style="margin-left:.5rem">📥 导入剧本</button></div></div>
  ...
  function _ij_updatePreview() {
    ...
    try {
      ...
      previewEl.innerHTML = `<div style="background:var(--bg3,#1a1e2e);padding:.6rem;border-radius:6px;font-size:.85rem">
        ...
        ${isAppend ? '<br><span style="color:var(--text-dim,#888)">将追加到当前活动项目，重复的镜头会报错</span>' : ''}
      </div>`;
    } catch (e) {
...
async function _showPromptGenerator(styles, genres, styleOpts, genreOpts) {
  ...
  // 加载提示词
  let _pg_mode = 'setup';  // setup / shots
