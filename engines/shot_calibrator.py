...
def calibrate_storyboard(llm: object, params: object) -> list[dict]:
    ...
    skeleton = llm_call_with_retry(llm, context, _STAGE1_SYSTEM, "叙事骨架", max_tokens=4096)
    ...
    enriched = _enrich_stage(llm, shots, _STAGE2_SYSTEM, "视觉描述", required_fields=["action", "dialogue"])
    if enriched:
        shots = enriched
    ...
    enriched = _enrich_stage(llm, shots, _STAGE3_SYSTEM, "AI 绘图", required_fields=["image_prompt_en"])
    if enriched:
        shots = enriched
...
def _enrich_stage(llm: object, shots: list[dict], system: str, label: str, required_fields: list[str] | None = None) -> list[dict] | None:
    ...
    if required_fields:
        ...
        if missing > len(merged) * 0.5:
            logger.warning(f"  {label}: 超过 50% 镜头缺少必填字段 {required_fields}，视为失败")
            return None
